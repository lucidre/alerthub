...
import 'package:alerthub/features/user/presentation/controller/ambulance_account_setup_controller.dart';
import 'package:alerthub/common_libs.dart';
import 'package:alerthub/shared/widgets/select_country.dart';
import 'package:country_picker/country_picker.dart';

@RoutePage()
class AmbulanceAccountSetupScreen extends StatefulWidget {
  final String email;
  const AmbulanceAccountSetupScreen({
    super.key,
    required this.email,
  });

  @override
  State<AmbulanceAccountSetupScreen> createState() =>
      _AmbulanceAccountSetupScreenState();
}

class _AmbulanceAccountSetupScreenState
    extends State<AmbulanceAccountSetupScreen> {
  ...
  void initState() {
    super.initState();
    Get.put(
      AmbulanceAccoutSetupController(
        UserService(
          UserRepositoryImpl(
  ...
  @override
  void dispose() {
    Get.delete<AmbulanceAccoutSetupController>(tag: tag);
    super.dispose();
  }
  ...
  void signUpUser() async {
    ...
    try {
      final controller = Get.find<AmbulanceAccoutSetupController>(tag: tag);
      await controller.signUpAmbulance();
          ...
          return TextFormField(
            ...
            validator: (value) {
              if (value == null || value.trim().isEmpty) {
                return 'Kindly provide your fullname.';
              }
  ...
  List<Widget> buildPhoneNumber() {
    return [
      ...
      Obx(() {
        ...
        final phoneNumberFocusNode = controller.phoneNumberFocusNode;
        final hospitalIdFocusNode = controller.hospitalIdFocusNode;

        return TextFormField(
          textInputAction: TextInputAction.next,
          focusNode: phoneNumberFocusNode,
          onFieldSubmitted: (_) =>
              FocusScope.of(context).requestFocus(hospitalIdFocusNode),
          decoration: context.inputDecoration(
  ...
  List<Widget> buildHospitalId() {
    return [
      Text('Hospital id', style: satoshi500S12).fadeInAndMoveFromBottom(),
      verticalSpacer8,
      Obx(() {
        final controller = Get.find<AmbulanceAccoutSetupController>(tag: tag);
        final hospitalIdController = controller.hospitalIdController;
        final hospitalIdFocusNode = controller.hospitalIdFocusNode;

        return TextFormField(
          textInputAction: TextInputAction.next,
          focusNode: hospitalIdFocusNode,
          decoration:
              context.inputDecoration(hintText: 'Enter your hospital id'),
          keyboardType: TextInputType.phone,
          validator: (value) {
            if (value == null || value.trim().isEmpty) {
              return 'Hospital id can not be empty.';
            }
            return null;
          },
