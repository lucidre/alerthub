...
class AppBottomNavigationBar extends StatelessWidget {
  final List<BottomBarModel> bottomBarModels;
  final int currentIndex;
  final Function(int)? onTap;

  const AppBottomNavigationBar({
    super.key,
    required this.bottomBarModels,
    this.currentIndex = 0,
    this.onTap,
  });
  @override
  Widget build(BuildContext context) {
    return Container(
      ...
      child: SafeArea(
        child: Row(
          mainAxisAlignment: bottomBarModels.length <= 2
              ? MainAxisAlignment.spaceEvenly
              : MainAxisAlignment.spaceBetween,
          children: [for (final item in bottomBarModels) buildItem(item)],
        ),
      ),
    );
  }
  ...
  buildItem(BottomBarModel item) {
    return TweenAnimationBuilder<double>(
      tween: Tween(
        end: bottomBarModels.indexOf(item) == currentIndex ? 1.0 : 0.0,
      ),
      curve: Curves.easeIn,
      ...
      builder: (context, value, _) {
        ...
        return Material(
          ...
          child: InkWell(
            onTap: () => onTap?.call(bottomBarModels.indexOf(item)),
